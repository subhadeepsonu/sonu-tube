...
export default async function Watchlater(){
    ...
    return <div className="min-h-screen w-full flex flex-col justify-start bg-gray-100 items-center pt-20 md:pl-20">
        ...
        {data.map((video)=>{
            return <VideoCard id={video.video.id} imgurl={video.video.thumnailurl} name={video.user.name} title={video.video.title} userimage={video.user.imgurl} views={video.video._count.views} >
