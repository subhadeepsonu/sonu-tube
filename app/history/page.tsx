...
export default async function HistoryPage(){
    ...
    return <div className="min-h-screen w-full flex flex-col justify-start bg-gray-100 items-center pt-20 md:pl-20">
        ...
        {data.map((history)=>{
                return <VideoCard key={history.id} id={history.video.id} imgurl={history.video.thumnailurl} name={history.video.user.name} title={history.video.title} userimage={history.video.user.imgurl} views={history.video._count.views}></VideoCard>
            })}
        </div>
    </div>      
}