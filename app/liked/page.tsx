...
export default async function LikedPage(){
    ...
    return <div className="min-h-screen w-full flex flex-col justify-start bg-gray-50 items-center pt-20 md:pl-20">
            <p className="text-2xl p-2 font-semibold">Liked Videos</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">                
                {data.map((video)=>{
                    return <VideoCard key={video.id} id={video.video.id} userimage={video.video.user.imgurl} name={video.video.user
